...
CREATE VIEW items AS SELECT products.name, catalogs.name 
...
SELECT a.created_at, (
	CASE 
		WHEN a.created_at = t.created_at THEN 1 ELSE 0 END) AS `Boolean` FROM august a 
JOIN tasktable t
...
DELETE FROM august 
WHERE created_at NOT IN (SELECT *FROM (SELECT created_at FROM august ORDER BY created_at DESC LIMIT 5));
